...
from constants import CHAIN_TO_NUMBER
from metrics import get_CA_and_sequence
...
def get_boltz_model(
    ...
    **kwargs,
    ...
    # Unpacking kwargs
    skip_run_structure = kwargs.pop("skip_run_structure", False)
    print("Skip Run Structure Prediction", skip_run_structure)
    ...
    if model_version == "boltz2":
        model_module = model_class.load_from_checkpoint(
            ...
            ema=False,
            structure_prediction_training=True if not skip_run_structure else False,
            no_msa=False,
...
def run_prediction(
    ...
    # 3.5: If template coords are provided for score-only evaluation
    if template_coords is not None:
        if isinstance(template_coords, np.ndarray):
            template_coords = torch.tensor(template_coords, dtype = torch.float32)
        
        # Changes shape from (Number of atoms, 3) to (1, Number of Atoms, 3) -> Bypass assertion check for 4-D inputs
        if len(template_coords.shape) == 2:
            template_coords = template_coords.unsqueeze(0)
        
        batch['coords'] = template_coords

    # 4. Move batch to device
    batch = {k: v.unsqueeze(0).to(device) for k, v in batch.items()}

    print("Token to Rep Atom Shape: ", batch["token_to_rep_atom"].shape)
